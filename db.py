...
class BotDB:
    statuses = {
        # Водитель
        'online': 'Доступен 🟢',  # Водитель доступен для заказов, не имеет текущего заказа
        'offline': 'Не доступен 🔴',  # Водитель не доступен для заказов, перерыв, отдых
        'route': 'Везет клиента 🟠',  # Водитель не доступен для заказов, имеет\выполняет текущий заказ

        # Заказ
        'create': 'Создан 🟡',  # Бот предлагает клиенту проверить заказ и подтвердить его
        'waiting': 'Ожидает машину 🟢',  # Заказ подтвержден клиентом, система предлагает водителям заказ
        'progress': 'В пути 🟠',  # Водитель выбран, клиент ожидает подачу машины либо клиент уже в пути
        'done': 'Выполнен 🔵',  # Клиент доставлен, водитель\клиент нажал кнопку "Заказ выполнен"
        'cancel': 'Отменен ⚫',  # Заказ не выполнен, водитель\клиент нажал кнопку "Отменить заказ"
    ...
    def update_client(self, user_id, data):
        ...
        try:
            self.cursor.execute(
                "UPDATE `user` SET name = " + self.replacer + ", phone = " + self.replacer + " WHERE tg_user_id = " + self.replacer,
    ...
    def user_update_tg_username(self, user_id, username):
        ...
        try:
            self.cursor.execute(
                "UPDATE `user` SET tg_username = " + self.replacer + " WHERE tg_user_id = " + self.replacer,
                (username, user_id))
        except Error as e:
            print(e)
        result = self.conn.commit()
    ...
    def user_update_balance(self, user_id, data):
        ...
        self.connect()
        try:
            self.cursor.execute("UPDATE `user` SET balance = " + self.replacer + " WHERE tg_user_id = " + self.replacer,
                                (data, user_id))
        except Error as e:
            print(e)
        result = self.conn.commit()
    ...
    def get_order_waiting_by_driver_id(self, driver_id):
        self.connect()
        self.cursor.execute("SELECT * FROM `order` WHERE driver_id = " + self.replacer + " AND status = 'waiting'",
                            (driver_id,))
    ...
    def get_orders(self, status):
        self.connect()
        self.cursor.execute("SELECT * FROM `order` WHERE `status` = " + self.replacer + " ORDER BY `dt_order`",
                            (status,))
    ...
    def get_client_orders(self, client_id):
        self.connect()
        self.cursor.execute("SELECT * FROM `order` WHERE `client_id` = " + self.replacer + " ORDER BY `dt_order` DESC",
                            (client_id,))
        result = self.cursor.fetchall()
        self.close()
        return result
    ...
    def get_client_orders_by_one(self, client_id, offset):
        self.connect()
        self.cursor.execute(
            "SELECT * FROM `order` WHERE `client_id` = " + self.replacer + " ORDER BY `dt_order` DESC LIMIT " + self.replacer + ", 1",
            (client_id, int(offset)))
        result = self.cursor.fetchall()
        self.close()
        return result
    ...
    def update_order_driver_id(self, order_id, driver_id):
        self.connect()
        try:
            self.cursor.execute("UPDATE `order` SET driver_id = " + self.replacer + " WHERE id = " + self.replacer,
                                (driver_id, order_id,))
        except Error as e:
            print(e)
        result = self.conn.commit()
    ...
    def cancel_all_orders_after_kicked_user(self, client_id):
        ...
        try:
            self.cursor.execute(
                "UPDATE `order` SET status = 'cancel' WHERE status IN ('create', 'waiting') AND client_id = " + self.replacer,
                (client_id,))
        except Error as e:
            print(e)
        result = self.conn.commit()
    ...
    def get_order_progress_by_driver_id(self, driver_id):
        self.connect()
        self.cursor.execute("SELECT * FROM `order` WHERE `driver_id` = " + self.replacer + " AND status = 'progress'",
                            (driver_id,))
        result = self.cursor.fetchone()
        self.close()
        return result
    ...
    def get_waiting_orders_by_client_id(self, client_id):
        self.connect()
        self.cursor.execute("SELECT * FROM `order` WHERE `client_id` = " + self.replacer + " AND status = 'waiting'",
                            (client_id,))
        result = self.cursor.fetchall()
        self.close()
        return result
    ...
    def get_create_order_by_client_id(self, client_id):
        self.connect()
        self.cursor.execute("SELECT * FROM `order` WHERE `client_id` = " + self.replacer + " AND status = 'create'",
                            (client_id,))
        result = self.cursor.fetchone()
        self.close()
        return result
    ...
    def get_done_orders_by_client_id(self, client_id):
        self.connect()
        self.cursor.execute("SELECT * FROM `order` WHERE `client_id` = " + self.replacer + " AND status = 'done'",
                            (client_id,))
        result = self.cursor.fetchall()
        self.close()
        return result
    ...
    def user_add(self, user_id, first_name, user_type):
        ...
        try:
            self.cursor.execute(
                "INSERT INTO `user` (`tg_user_id`, `tg_first_name`, `wallet`, `user_type`) VALUES (" + self.replacer + ", " + self.replacer + ", " + self.replacer + ", " + self.replacer + ")",
                (user_id, first_name, user_id, user_type))
        except Error as e:
            print(e)
        result = self.conn.commit()
        self.close()
    ...
    def user_get(self, user_id, user_type):
        ...
        self.connect()
        self.cursor.execute(
            "SELECT * FROM `user` WHERE tg_user_id = " + self.replacer + " AND user_type = " + self.replacer,
            (user_id, user_type))
        result = self.cursor.fetchone()
        self.close()
        return result
    ...
    def add_driver(self, user_id, first_name):
        ...
        try:
            self.cursor.execute(
                "INSERT INTO `user` (`tg_user_id`, `tg_first_name`, `wallet`) VALUES (" + self.replacer + ", " + self.replacer + ", " + self.replacer + ")",
    ...
    def update_driver_wallet(self, user_id, data):
        ...
        self.connect()
        try:
            self.cursor.execute("UPDATE `user` SET wallet = " + self.replacer + " WHERE tg_user_id = " + self.replacer,
                                (data, user_id))
        except Error as e:
            print(e)
        result = self.conn.commit()
    ...
    def update_driver(self, user_id, data):
        ...
        try:
            self.cursor.execute(
                "UPDATE `user` SET name = " + self.replacer + ", phone = " + self.replacer + ", car_number = " + self.replacer + ", status = " + self.replacer + " WHERE tg_user_id = " + self.replacer,
                (data['name'], data['phone'], data['car_number'], data['status'], user_id))
        except Error as e:
            print(e)
        result = self.conn.commit()
        self.close()
    ...
    def update_driver_referer(self, user_id, referer_user_id):
        ...
        try:
            self.cursor.execute(
                "UPDATE `user` SET referer_user_id = " + self.replacer + " WHERE tg_user_id = " + self.replacer,
    ...
    def update_driver_status(self, user_id, status):
        ...
        self.connect()
        try:
            self.cursor.execute("UPDATE `user` SET status = " + self.replacer + " WHERE tg_user_id = " + self.replacer,
                                (status, user_id))
        except Error as e:
            print(e)
        result = self.conn.commit()
    ...
    def update_driver_location(self, user_id, latitude, longitude):
        ...
        try:
            self.cursor.execute(
                "UPDATE `user` SET latitude = " + self.replacer + ", longitude = " + self.replacer + " WHERE tg_user_id = " + self.replacer,
    ...
    def update_driver_type(self, user_id, new_type):
        ...
        try:
            self.cursor.execute(
                "UPDATE `user` SET user_type = " + self.replacer + " WHERE tg_user_id = " + self.replacer,
                (new_type, user_id))
        except Error as e:
            print(e)
        result = self.conn.commit()
    ...
    def driver_order_exists(self, driver_id, order_id):
        self.connect()
        self.cursor.execute(
            "SELECT `driver_id` FROM `driver_order` WHERE `driver_id` = " + self.replacer + " AND order_id = " + self.replacer,
            (driver_id, order_id))
    ...
    def driver_order_create(self, driver_id, order_id):
        ...
        try:
            self.cursor.execute(
                "INSERT INTO `driver_order` (`driver_id`, `order_id`, `driver_cancel_cn`) VALUES (" + self.replacer + ", " + self.replacer + ", 0)",
                (driver_id, order_id,))
        except Error as e:
            print(e)
        result = self.conn.commit()
    ...
    def driver_order_increment_cancel_cn(self, driver_id, order_id):
        ...
        try:
            self.cursor.execute(
                "UPDATE `driver_order` SET driver_cancel_cn = IFNULL(driver_cancel_cn, 0) + 1 WHERE driver_id = " + self.replacer + " AND order_id = " + self.replacer,
                (driver_id, order_id))
            result = self.conn.commit()
        except Error as e:
            print(e)
    ...
    def get_driver_order(self, driver_id, order_id):
        self.connect()
        self.cursor.execute(
            "SELECT * FROM `driver_order` WHERE `driver_id` = " + self.replacer + " AND order_id = " + self.replacer,
            (driver_id, order_id))
        result = self.cursor.fetchone()
        self.close()
        return result
    ...
    def get_location_by_name(self, text):
        self.connect()
        sql = (
                    "SELECT * FROM location" + DB_LOCATION_POSTFIX + " WHERE `name_rus` LIKE '%{text:s}%' OR name_eng LIKE '%{text:s}%' OR search_rus LIKE '%{text:s}%'")
        sql = sql.format(
            text=text
        )
        self.cursor.execute(sql)
    ...
    def get_locations_by_category_id(self, category_id):
        self.connect()
        sql = (
                    "SELECT * FROM location" + DB_LOCATION_POSTFIX + " WHERE `category_id` = {category_id:d} ORDER BY name_rus")
        sql = sql.format(
    ...
    def get_location_by_id(self, location_id):
        self.connect()
        self.cursor.execute("SELECT * FROM location" + DB_LOCATION_POSTFIX + " WHERE `id` = " + self.replacer,
                            (location_id,))
        result = self.cursor.fetchone()
        self.close()
        return result
