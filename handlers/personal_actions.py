...
ℹ️ Для возможности принимать заказы должна быть заполнена АНКЕТА и пополнен БАЛАНС. Также ознакомьтесь с условиями взимания комиссии
 ...
 • Баланс
    ℹ️ До 1 марта 2023 года водителям будут начислен бонусный баланс в размере 20 USDT для возможности принимать заказы. Бонусный баланс не доступен для вывода.
    ...
    👉 Баланс служит для списания комиссии, согласно установленных тарифов, с водителей за выполненные заказы. Списание баланса происходит в момент принятия заказа
    👉 Если вам потребуется вернуть внесенные денежные средства, напишите {adminTg:s}
...
async def incentiveDriverFillForm(message):
    ...
    await message.bot.send_message(message.from_user.id, 'Сообщение доставлено ' + str(len(unregisteredDriverModels)) + ' пользователям')
